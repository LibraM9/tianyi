...
def create_feature(data):
    '''
    提取特征：所有历史分数的均值、中位数、方差、最大值、最小值、近2次的均值、近2次的方差
    '''
    feats = []
    for i, row in tqdm(data.iterrows()):
        ...
        feats.append([np.mean(m), np.median(m), np.std(m), np.max(m), np.min(m),
                      ...
                      # np.mean(m[-5:]), np.std(m[-5:]),np.mean(m[-3:]), np.std(m[-3:]),
                      # np.mean(m[-3:])/np.mean(m[-5:]),np.mean(m[-3:])/np.mean(m)
                      ])
...
'''
test_id = list(set(test['student_id']))
traindata = []
for stu in tqdm(test_id):
    history_grade = train[train.student_id == stu]
    student_test = test[test.student_id == stu]
...
traindata = traindata.merge(student, how='left', on='student_id') #性别
...
##标记1和2，方便结合数据：
test['one'] = test['exam_id'].apply(lambda x: 1 if x in exam_id1 else 2)
print(test['one'].value_counts())
...
result.to_csv(out_path+'result512.csv', index=None, encoding='utf8')